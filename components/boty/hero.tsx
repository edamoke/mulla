...
import { CMSHero } from "@/lib/cms-store"
...
export function Hero({ cms }: HeroProps) {
  const subtitle = cms?.subtitle ?? "Luxury Lifestyle in Malindi"
  const title1 = cms?.title_line1 ?? "Live elegantly."
  const title2 = cms?.title_line2 ?? "Uniquely you."
  const description = cms?.description ?? "Discover curated fashion, imported furniture, and exquisite decor. Premium collections for the discerning coastal lifestyle."
  const buttonText = cms?.button_text ?? "Shop Collection"
  const videoUrl = cms?.video_url ?? "https://hebbkx1anhila5yf.public.blob.vercel-storage.com/f3d8cad2-8091-4809-aac0-eaac74b0be7c-Z4XUCz3CRR7qjaOsoq6rFmbJfIRdgs.mp4"
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <div className="w-full lg:max-w-xl mx-auto lg:mx-0 text-center lg:text-left">
            ...
            <h2 className="font-serif text-5xl md:text-6xl lg:text-7xl leading-[1.1] mb-6 text-balance text-black">
              <span className="block animate-blur-in opacity-0 font-semibold" style={{ animationDelay: '0.4s', animationFillMode: 'forwards' }}>{title1}</span>
              <span className="block animate-blur-in opacity-0 font-semibold xl:text-9xl text-7xl" style={{ animationDelay: '0.6s', animationFillMode: 'forwards' }}>{title2}</span>
            </h2>
